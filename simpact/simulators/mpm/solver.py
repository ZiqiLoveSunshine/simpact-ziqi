...
class MPM_Simulator_WARP:
    ...
    def add_box_collider(self, box_min, box_max, surface="sticky", 
                         friction=0.0, velocity = (0.0, 0.0, 0.0),
                         start_time=0.0, end_time=999.0):
        collider_param = Dirichlet_collider()
        center = [ (box_max[0] + box_min[0]) / 2.0, 
        ...
        collider_param.velocity = wp.vec3(velocity[0], velocity[1], velocity[2])
        collider_param.width = box_max[0] - box_min[0]
        collider_param.height = box_max[1] - box_min[1]
        collider_param.length = box_max[2] - box_min[2]
        ...
        def collide(
            ...
            if time >= param.start_time and time < param.end_time:
                ...
                world_x = float(grid_x) * model.dx
                world_y = float(grid_y) * model.dx
                world_z = float(grid_z) * model.dx
                
                # Calculate box boundaries in world coordinates
                box_min_x = param.point[0] - param.width / 2.0
                box_max_x = param.point[0] + param.width / 2.0
                box_min_y = param.point[1] - param.height / 2.0
                box_max_y = param.point[1] + param.height / 2.0
                box_min_z = param.point[2] - param.length / 2.0
                box_max_z = param.point[2] + param.length / 2.0
                
                # Check if point is inside the box
                inside_x = world_x >= box_min_x and world_x <= box_max_x
                inside_y = world_y >= box_min_y and world_y <= box_max_y
                inside_z = world_z >= box_min_z and world_z <= box_max_z
                
                if inside_x and inside_y and inside_z:
                    # Point is inside the box, determine which face is closest
                    dist_to_min_x = world_x - box_min_x
                    dist_to_max_x = box_max_x - world_x
                    dist_to_min_y = world_y - box_min_y
                    dist_to_max_y = box_max_y - world_y
                    dist_to_min_z = world_z - box_min_z
                    dist_to_max_z = box_max_z - world_z
                    
                    # Find minimum distance to determine closest face
                    ...
                    # Apply collision based on surface type
                    v = state.grid_v_out[grid_x, grid_y, grid_z]
                    normal_component = wp.dot(v, normal)
                    
                    if param.surface_type == 0:
                        # Stick - zero velocity
                        state.grid_v_out[grid_x, grid_y, grid_z] = param.velocity
                    elif param.surface_type == 1:
                        # Slip - project out all normal component
                        v = v - normal_component * normal
                        state.grid_v_out[grid_x, grid_y, grid_z] = v
                    # else:
                    #     # Separable - project out only inward normal component
                    #     v = v - wp.min(normal_component, 0.0) * normal
                    #     if normal_component < 0.0 and wp.length(v) > 1e-20:
                    #         # Apply friction
    ...
    def add_cylinder_collider(self, point, radius, surface="sticky", 
                              friction=0.0, velocity = (0.0, 0.0, 0.0),
                              start_time=0.0, end_time=999.0):
        collider_param = Dirichlet_collider()
        ...
        def collide(
            ...
            if time >= param.start_time and time < param.end_time:
                ...
                delta_x = param.point[0] - world_x
                delta_y = param.point[1] - world_y
                dist_xy = wp.sqrt(delta_x * delta_x + delta_y * delta_y)
                
                if dist_xy < param.R:
                    
                    delta_vec = wp.vec3(delta_x, delta_y, 0.0)
                    delta_vec = delta_vec / (dist_xy + 1e-10) 
    ...
    def add_rotate_box_collider(self, point, quat, twist, 
                                surface="sticky", friction=0.0, 
                                width=1.0, height=1.0, length=1.0,
                                start_time=0.0, end_time=999.0):
        collider_param = Dirichlet_collider()
        ...
        def collide(
            ...
            if time >= param.start_time and time < param.end_time:
                ...
                world_points = wp.vec3(world_x, world_y, world_z)
                
                # Transform world point to local box coordinates
                local_points = wp.transform_point(wp.transform_inverse(param.xform), world_points)
                
                local_x = local_points[0]
                local_y = local_points[1]
                local_z = local_points[2]

                # ADD PADDING: Extendcollision boundary outward
                collision_padding = 1.0 * model.dx  # 2-3 grid cells recommended
                
                # Check if point is inside the EXTENDED box (in local coordinates)
                inside_x = local_x >= -(param.width/2.0 + collision_padding) and local_x <= (param.width/2.0 + collision_padding)
                inside_y = local_y >= -(param.height/2.0 + collision_padding) and local_y <= (param.height/2.0 + collision_padding)
                inside_z = local_z >= -(param.length/2.0 + collision_padding) and local_z <= (param.length/2.0 + collision_padding)
                
                if inside_x and inside_y and inside_z:
                    # Point is inside the extended box, determine which face is closest
                    dist_to_min_x = local_x - (-(param.width/2.0 + collision_padding))
                    dist_to_max_x = (param.width/2.0 + collision_padding) - local_x
                    dist_to_min_y = local_y - (-(param.height/2.0 + collision_padding))
                    dist_to_max_y = (param.height/2.0 + collision_padding) - local_y
                    dist_to_min_z = local_z - (-(param.length/2.0 + collision_padding))
                    dist_to_max_z = (param.length/2.0 + collision_padding) - local_z

                # # Check if point is inside the box (in local coordinates)
                # inside_x = local_x >= -param.width/2.0 and local_x <= param.width/2.0
                # inside_y = local_y >= -param.height/2.0 and local_y <= param.height/2.0
                # inside_z = local_z >= -param.length/2.0 and local_z <= param.length/2.0
                
                # if inside_x and inside_y and inside_z:
                #     # Point is inside the box, determine which face is closest (in local coordinates)
                #     dist_to_min_x = local_x - (-param.width/2.0)
                #     dist_to_max_x = param.width/2.0 - local_x
                #     dist_to_min_y = local_y - (-param.height/2.0)
                #     dist_to_max_y = param.height/2.0 - local_y
                #     dist_to_min_z = local_z - (-param.length/2.0)
                #     dist_to_max_z = param.length/2.0 - local_z
                    
                    # Find minimum distance to determine closest face (local normal)
                    min_dist = dist_to_min_x
                    local_normal = wp.vec3(-1.0, 0.0, 0.0)
                    ...
                    if dist_to_min_y < min_dist:
                        min_dist = dist_to_min_y
                        local_normal = wp.vec3(0.0, -1.0, 0.0)
                    if dist_to_max_y < min_dist:
                        min_dist = dist_to_max_y
                        local_normal = wp.vec3(0.0, 1.0, 0.0)
                    if dist_to_min_z < min_dist:
                        min_dist = dist_to_min_z
                        local_normal = wp.vec3(0.0, 0.0, -1.0)
                    if dist_to_max_z < min_dist:
                        min_dist = dist_to_max_z
                        local_normal = wp.vec3(0.0, 0.0, 1.0)
                    
                    # Transform local normal to world coordinates
                    world_normal = wp.transform_vector(param.xform, local_normal)
                    
                    # Get velocity at this grid point
                    v = state.grid_v_out[grid_x, grid_y, grid_z]
                    
                    # Calculate velocity of the collider surface at this point
                    # velocity = linear_velocity + angular_velocity × offset
                    collider_linear_vel = wp.vec3(param.twist[3], param.twist[4], param.twist[5])
                    collider_angular_vel = wp.vec3(param.twist[0], param.twist[1], param.twist[2])
                    offset_from_center = world_points - wp.transform_get_translation(param.xform)
                    collider_velocity = collider_linear_vel + wp.cross(collider_angular_vel, offset_from_center)
                    
                    # Relative velocity
                    relative_v = v - collider_velocity
                    normal_component = wp.dot(relative_v, world_normal)
                    
                    if param.surface_type == 0:
                        # Stick - match collider velocity
                        state.grid_v_out[grid_x, grid_y, grid_z] = collider_velocity
                    elif param.surface_type == 1:
                        # Slip - project out all normal component of relative velocity
                        relative_v = relative_v - normal_component * world_normal
                        state.grid_v_out[grid_x, grid_y, grid_z] = relative_v + collider_velocity
                    else:
                        # Separable - project out only inward normal component
                        relative_v = relative_v - wp.min(normal_component, 0.0) * world_normal
                        if normal_component < 0.0 and wp.length(relative_v) > 1e-20:
                            # Apply friction
                            relative_v = wp.max(0.0, wp.length(relative_v) + normal_component * param.friction) * wp.normalize(relative_v)
                        state.grid_v_out[grid_x, grid_y, grid_z] = relative_v + collider_velocity
        ...
        # def modify(time, dt, param: Dirichlet_collider):
        #     if time >= param.start_time and time < param.end_time:
        #         # Update transform based on twist (spatial velocity)
        #         param.xform = wp.transform_multiply(
        #             param.xform,
        #             wp.transform(
        #                 [dt * param.twist[3], dt * param.twist[4], dt * param.twist[5]],  # linear displacement
        #                 wp.quat_from_axis_angle(
        #                     wp.normalize(wp.vec3(param.twist[0], param.twist[1], param.twist[2])),
        #                     wp.length(wp.vec3(param.twist[0], param.twist[1], param.twist[2])) * dt
        #                 ) if wp.length(wp.vec3(param.twist[0], param.twist[1], param.twist[2])) > 1e-10 
        ...
        def modify(time, dt, param: Dirichlet_collider):
            if time >= param.start_time and time < param.end_time:
                ...
                angular_vel_world = wp.vec3(param.twist[0], param.twist[1], param.twist[2])
                angular_speed = wp.length(angular_vel_world)
                
                if angular_speed > 1e-10:
                    # Create rotation quaternion in world frame
                    rotation_axis_world = wp.normalize(angular_vel_world)
                    rotation_angle = angular_speed * dt
                    delta_rotation_world = wp.quat_from_axis_angle(rotation_axis_world, rotation_angle)
                else:
                    delta_rotation_world = wp.quat_identity()
                
                # Linear velocity in WORLD frame
                linear_vel_world = wp.vec3(
                    param.twist[3], 
                    param.twist[4], 
                    param.twist[5]
                )
                ...
                current_pos = wp.transform_get_translation(param.xform)
                current_rot = wp.transform_get_rotation(param.xform)
                
                # Quaternion multiplication: q1 * q2 (world frame rotation)
                ...
                # For world-frame rotation: new_rotation = delta_rotation * current_rotation
                q1_x = delta_rotation_world[0]
                q1_y = delta_rotation_world[1]
                q1_z = delta_rotation_world[2]
                q1_w = delta_rotation_world[3]
                ...
                new_rot_x = q1_w * q2_x + q1_x * q2_w + q1_y * q2_z - q1_z * q2_y
                new_rot_y = q1_w * q2_y - q1_x * q2_z + q1_y * q2_w + q1_z * q2_x
                new_rot_z = q1_w * q2_z + q1_x * q2_y - q1_y * q2_x + q1_z * q2_w
                new_rot_w = q1_w * q2_w - q1_x * q2_x - q1_y * q2_y - q1_z * q2_z
                
                new_rotation = wp.quat(new_rot_x, new_rot_y, new_rot_z, new_rot_w)
                
                # Update position with world-frame linear velocity
                new_position = current_pos + linear_vel_world * dt
                
                # Create new transform
                param.xform = wp.transform(
                    [new_position[0], new_position[1], new_position[2]],
                    [new_rotation[0], new_rotation[1], new_rotation[2], new_rotation[3]]
                )
